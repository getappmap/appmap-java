...
// import com.appland.appmap.annotation.Label;
...
public class NHttpReverseProxy {
    ...
    static class ThrowOnRequest implements HttpRequestInterceptor {
        public void process(HttpRequest req, HttpContext ctx) throws HttpException {
            throw new HttpException("nope");
        }
    }
    ...
    public static void main(final String[] args) throws Exception {
        if (args.length < 2) {
            System.out.println(
                    "Usage: NHttpReverseProxy <HostNameURI> <Port> [\"TrustSelfSignedStrategy\"]");
            System.exit(1);
        }
        ...
        final HttpHost targetHost =
                new HttpHost(uri.getHost(), uri.getPort() > 0 ? uri.getPort() : 80,
                        uri.getScheme() != null ? uri.getScheme() : "http");

        System.out.println("Reverse proxy to " + targetHost);

        final ConnectingIOReactor connectingIOReactor =
                new DefaultConnectingIOReactor(IOReactorConfig.custom().setIoThreadCount(1)
                        .setSoTimeout(5000).setConnectTimeout(5000).build());
        final ListeningIOReactor listeningIOReactor = new DefaultListeningIOReactor(
                IOReactorConfig.custom().setIoThreadCount(1).setSoTimeout(30000).build());

        // Set up HTTP protocol processor for incoming connections
        final HttpProcessor inhttpproc = new ImmutableHttpProcessor(new ResponseDate(),
                new ResponseServer("Test/1.1"), new ResponseContent(), new ResponseConnControl());

        // Set up HTTP protocol processor for outgoing connections
        final HttpProcessor outhttpproc = new ImmutableHttpProcessor(new RequestContent(),
                new RequestTargetHost(), new RequestConnControl(), new RequestUserAgent("Test/1.1"),
                new RequestExpectContinue(true));

        final ProxyClientProtocolHandler clientHandler = new ProxyClientProtocolHandler();
        final HttpAsyncRequester executor =
                new HttpAsyncRequester(outhttpproc, new ProxyOutgoingConnectionReuseStrategy());

        // Without SSL: ProxyConnPool connPool = new ProxyConnPool(connectingIOReactor,
        ...
        final ProxyConnPool connPool = new ProxyConnPool(connectingIOReactor,
                new BasicNIOConnFactory(
                        ...
                        new SSLNHttpClientConnectionFactory(sslContext, null,
                                ConnectionConfig.DEFAULT)),
                0);
        connPool.setMaxTotal(100);
        connPool.setDefaultMaxPerRoute(20);

        final UriHttpAsyncRequestHandlerMapper handlerRegistry =
                new UriHttpAsyncRequestHandlerMapper();
        handlerRegistry.register("*", new ProxyRequestHandler(targetHost, executor, connPool));

        final ProxyServiceHandler serviceHandler = new ProxyServiceHandler(inhttpproc,
                new ProxyIncomingConnectionReuseStrategy(), handlerRegistry);

        final IOEventDispatch connectingEventDispatch = DefaultHttpClientIODispatch
                .create(clientHandler, sslContext, ConnectionConfig.DEFAULT);

        final IOEventDispatch listeningEventDispatch =
                new DefaultHttpServerIODispatch<NHttpServerEventHandler>(serviceHandler,
                        ConnectionConfig.DEFAULT);
        ...
        public void handle(final ProxyHttpExchange httpExchange,
            ...
            synchronized (httpExchange) {
                ...
                if (response != null) {
                    ...
                    System.out.println(
                            "[client<-proxy] " + httpExchange.getId() + " response triggered");
                }
                // No response yet.
    ...
    static class ProxyRequestConsumer implements HttpAsyncRequestConsumer<ProxyHttpExchange> {
        ...
        public void consumeContent(final ContentDecoder decoder, final IOControl ioControl)
                throws IOException {
            synchronized (this.httpExchange) {
                ...
                System.out.println(
                        "[client->proxy] " + this.httpExchange.getId() + " " + n + " bytes read");
                if (decoder.isCompleted()) {
                    System.out.println(
                            "[client->proxy] " + this.httpExchange.getId() + " content fully read");
                }
        ...
        public HttpRequest generateRequest() {
            synchronized (this.httpExchange) {
                ...
                if (request instanceof HttpEntityEnclosingRequest) {
                    final BasicHttpEntityEnclosingRequest r =
    ...
    static class ProxyResponseConsumer implements HttpAsyncResponseConsumer<ProxyHttpExchange> {
        ...
        public void responseReceived(final HttpResponse response) {
            synchronized (this.httpExchange) {
                ...
                if (responseTrigger != null && !responseTrigger.isCompleted()) {
                    System.out.println(
                            "[client<-proxy] " + this.httpExchange.getId() + " response triggered");
        ...
        public void consumeContent(final ContentDecoder decoder, final IOControl ioControl)
            ...
            synchronized (this.httpExchange) {
                ...
                final int n = decoder.read(buf);
                System.out.println(
                        "[proxy<-origin] " + this.httpExchange.getId() + " " + n + " bytes read");
                if (decoder.isCompleted()) {
                    System.out.println(
                            "[proxy<-origin] " + this.httpExchange.getId() + " content fully read");
                }
        ...
        public void responseCompleted(final HttpContext context) {
            synchronized (this.httpExchange) {
                ...
                this.completed = true;
                System.out.println(
                        "[proxy<-origin] " + this.httpExchange.getId() + " response completed");
        ...
        public void failed(final Exception ex) {
            synchronized (this.httpExchange) {
                ...
                if (responseTrigger != null && !responseTrigger.isCompleted()) {
                    ...
                    final int status =
                            ex instanceof SocketTimeoutException ? HttpStatus.SC_GATEWAY_TIMEOUT
    ...
    static class ProxyResponseProducer implements HttpAsyncResponseProducer {
        ...
        public void responseCompleted(final HttpContext context) {
            synchronized (this.httpExchange) {
                System.out.println(
                        "[client<-proxy] " + this.httpExchange.getId() + " response completed");
            }
        }
